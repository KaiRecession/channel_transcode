...
def train_test():
        ...
        for epi_counter in range(2):
            ...
            env.energy.set_buffer_transcode(env, 1, bit_rate)
            ...
            while not done:
                ...
                logits, logits_length, _ = server(tf.constant(current_state, dtype=tf.float32))
                # print(logits)
    ...
    for epi_counter in range(1):
        ...
        while not done:
            ...
            for _ in range(length):
                ...
                delay, sleep_time, buffer_size, rebuf, \
                video_chunk_size, next_video_chunk_sizes, \
                end_of_video, video_chunk_remain, energy, next_video_chunk_buffer_status = env.get_video_chunk(bit_rate)
                reward = VIDEO_BIT_RATE[bit_rate] / 1000.0 \
                         - 10 * rebuf \
